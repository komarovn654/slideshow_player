...
ssp_static int ssp_obsps_add_directories(void)
{
    for (size_t i = 0; i < obs_inotify.obs->dirs_count; i++) {
        ...
        if (obs_inotify.watch_descriptor[i] <= 0) {
            ssp_syslog(LOG_ERR, "SSP IObserver. Couldn't add to observer inotify <%s>: %s", obs_inotify.obs->dirs[i], 
                strerror(errno));
            return 1;
        }
    }
...
int ssp_obsps_init(ssp_observer settings)
{
    if ((obs_inotify.obs = ssp_obs_init(settings)) == NULL) {
        ssp_syslog(LOG_ERR, "SSP IObserver. Common observer initialization error");
        return 1;
    }

    if ((obs_inotify.inotify_instance = inotify_init()) < 0) {
        ssp_syslog(LOG_ERR, "SSP IObserver. Inotify initialization error: %s", strerror(errno));
        return 1;
    }

    if ((ssp_obs_dirs_create(obs_inotify.obs)) != 0) {
        ssp_syslog(LOG_ERR, "SSP IObserver. Observer directories creation error");
        return 1;
    }
    ...
    if (ssp_obsps_add_directories() != 0) {
        ssp_syslog(LOG_ERR, "SSP IObserver. Failed to sign catalogs on inotify");
        return 1;
    }
    ...
    if (ssp_obs_dirs_traversal(obs_inotify.obs) != 0) {
        ssp_syslog(LOG_ERR, "SSP IObserver. Directory traversal error");
        return 1;
    }

    for (size_t i = 0; i < obs_inotify.obs->dirs_count; i++) {
        ssp_syslog(LOG_INFO, "SSP IObserver. Observing: %s", obs_inotify.obs->dirs[i]);
    }
...
void ssp_obsps_destruct(void)
{
    ...
    ssp_syslog(LOG_INFO, "SSP IObserver. IObserver was destructed");
}
...
ssp_static void ssp_obsps_event_handle(uint32_t mask, const char* event_name)
{
    ...
    case IN_CREATE:
        if (obs_inotify.obs->filter(event_name) == false) {
            ssp_syslog(LOG_WARNING, "SSP IObserver. <%s> was filtred", event_name);
            break;
        }

        ssp_obs_storage_insert(obs_inotify.obs, event_name);
        ssp_syslog(LOG_INFO, "SSP IObserver. The file <%s> has been moved to the storage", event_name);
        break;
    case IN_MOVED_FROM:
    case IN_DELETE:
        ssp_obs_storage_remove(obs_inotify.obs, event_name);
        ssp_syslog(LOG_INFO, "SSP IObserver. The file <%s> has been deleted", event_name);
        break;
    default:
        ssp_syslog(LOG_WARNING, "SSP IObserver. Something(%d) has happened with <%s>", mask, event_name);
        break;
    }
}
...
int ssp_obsps_process(void)
{
    if (poll(&obs_inotify.fd, 1, 0) < 0) {
        ssp_syslog(LOG_ERR, "SSP IObserver. Obeserver couldn't poll(): %s", strerror(errno));
        return 1;
    }
    ...
    if (obs_inotify.fd.revents & POLLIN){
        ...
        if ((length = read(obs_inotify.fd.fd, buffer, SSP_OBS_EVENT_BUFFER_SIZE)) > 0) {
            ...
            while (IN_EVENT_OK(event, length)) {
                ...
                if (wd_index < 0) {
                    ssp_syslog(LOG_WARNING, "SSP IObserver. Observer catched an event from unknown wd: %i", event->wd);
                ...
                if (ssp_dir_absolute_path(obs_inotify.obs->dirs[wd_index], dirname) != 0) {
                    ssp_syslog(LOG_WARNING, "SSP IObserver. <%s> is too long and was truncated", dirname);
                    event = IN_EVENT_NEXT(event, length);
                    continue;
                }
                ...
                if (snprintf(fullname, SSP_FULL_NAME_MAX_LEN, "%s/%s", dirname, event->name) >= SSP_FULL_NAME_MAX_LEN) {
                    ssp_syslog(LOG_WARNING, "SSP IObserver. <%s> is too long and was truncated", fullname);
                    event = IN_EVENT_NEXT(event, length);
                    continue;
                };
