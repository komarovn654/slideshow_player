...
{
    syslog(LOG_ERR, "SSP. GLFW Error [%i]: %s\n", error, description);
}
...
ssp_static void ssp_window_resize_handler(int image_width, int image_height)
{
    ...
    syslog(LOG_DEBUG, "SSP. Image has been resized to %ix%i", res_width, res_height);
}
...
ssp_static void ssp_window_set_platform_name(int glfw_platform_id, char *name, int name_size) {
    ...
    case GLFW_PLATFORM_WIN32:
        ...
        break;         
    default:
        syslog(LOG_WARNING, "SSP. An unknown display platform type");
        break;
    }
}
...
ssp_static int ssp_window_set_platform(ssp_display_platform platform) {
    ...
    if (glfwPlatformSupported(platform) != 1) {
        syslog(LOG_ERR, "SSP. %s platform doesn't support", platform_name);
        return 1;
    }   
...
ssp_static int ssp_glfw_init(ssp_display_platform platform)
{
    ...
    syslog(LOG_INFO, "SSP. GLFW runtime  version: %s", glfwGetVersionString());
    syslog(LOG_INFO, "SSP. GLFW compiled version: %i.%i.%i", 
        GLFW_VERSION_MAJOR, GLFW_VERSION_MINOR, GLFW_VERSION_REVISION);

    if (ssp_window_set_platform(platform) != 0) {
        syslog(LOG_ERR, "SSP. Couldn't set display platform");
        return -1;
    }

    if (!glfwInit()) {
        syslog(LOG_ERR, "SSP. GLFW initialization error");
        return -1;
    }

    char platform_name[10];
    ssp_window_set_platform_name(glfwGetPlatform(), platform_name, sizeof(platform_name));
    syslog(LOG_INFO, "SSP. GLFW has been initializated for %s", platform_name);
...
int ssp_window_init(int width, int height, double redraw_time, ssp_image_storage* images)
{
    ...
    if (ssp_window.window == NULL) {
        syslog(LOG_ERR, "SSP. GLFW couldn't create window");
        return 1;
    }
    ...
    if (ssp_render_init(ssp_window_resize_handler) != 0) {
        syslog(LOG_ERR, "SSP. Render initialization error");
        return 1;        
    }
    syslog(LOG_INFO, "SSP. OpenGL version: %s", glGetString(GL_VERSION));
    syslog(LOG_INFO, "SSP. The window was initialized");
...
{
    glfwTerminate();
    syslog(LOG_INFO, "SSP. Window was destructed");
}

int ssp_window_player_loop(void)
{   
    if (glfwWindowShouldClose(ssp_window.window)) {
        syslog(LOG_INFO, "SSP. Window was closed");
        return 1;
    }
    ...
    if (ssp_window_needs_to_redraw()) {
        ...
        if (ssp_render_redraw(image) != 0) {
            syslog(LOG_ERR, "SSP. Redraw error: %s", image);
            return 0;
        }

        glfwSwapBuffers(ssp_window.window);
        syslog(LOG_INFO, "SSP. Readrawed: %s", image);
